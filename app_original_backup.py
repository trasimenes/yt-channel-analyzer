...
def calculate_total_views(videos):
    ...
    for video in videos:
        try:
            ...
            print(f"[WARNING] Error parsing views for video {video.get('id', 'unknown')}: {e}")
            continue
    return total
...
def format_number(num):
    ...
    try:
        ...
        print(f"[WARNING] Error formatting number '{num}': {e}")
        return str(num)
    ...
    if not playlists:
        ...
        if last_update:
            try:
                ...
                should_update_playlists = True
                print(f"[AUTO-UPDATE-PLAYLISTS] Erreur de date pour {competitor_name}: {e}, mise à jour automatique...")
...
def calculate_country_7_metrics(country):
    """
    Calculate 7 key metrics for a country - refactored using service layer architecture.
    Complexity reduced from 58 to ~5 by extracting business logic into specialized services.
    """
    ...
    try:
        ...
        metrics = service.calculate_country_7_metrics(country)
        
        return metrics
        
    except Exception as e:
        print(f"[COUNTRY_METRICS] Error calculating metrics for {country}: {e}")
        return service._empty_country_metrics() if 'service' in locals() else {}
    finally:
        if 'conn' in locals():
            conn.close()
...
if __name__ == '__main__':
    ...
    host = os.getenv('FLASK_HOST', '127.0.0.1')  # Localhost only by default for security
