...
def create_database():
    ...
    # Créer la table pour les données d'abonnés importées via CSV
    cursor.execute('''
        CREATE TABLE subscriber_data (
            ...
            FOREIGN KEY (concurrent_id) REFERENCES concurrent (id) ON DELETE CASCADE,
            UNIQUE(concurrent_id, date)
        )
    ''')
    
    # Index pour la nouvelle table
    cursor.execute('CREATE INDEX idx_subscriber_data_concurrent_id ON subscriber_data(concurrent_id)')
    cursor.execute('CREATE INDEX idx_subscriber_data_date ON subscriber_data(date)')
    ...
    print("   - subscriber_data (données d'abonnés importées)")
